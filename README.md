...
[![Version](https://img.shields.io/badge/version-v0.8.0-brightgreen)](CHANGELOG.md)
...
传统执行器（C/Go 编译出的机器码）是"哑执行器"——跑完就完了，它自己不知道跑的是什么。免语言执行器可以 `--trace` 边跑边懂，从四个维度摊开"执行器每一步怎么理解输入"：

- **规则层（how）**：它遵守什么规则——`1+1=2` 该标强值，标错即规则层错
- **执行层（what）**：它执行/理解有没有错——证明"非代码自身原因，是它的理解问题"
- **实际层（actual）**：客观事实——它看见/拿到什么（值对但位置错/外部污染）
- **看见层（see）**：它自己的判断——判断真假/强度（抓"自身判断失误"）

四层任意组合开关（如 `--trace=how,actual,what` 同时开多个、关一个）。关键区别：实际层 = 客观事实，看见层 = 主观判断，两者"像"但不同根，不合并。
...
更进一步——`boot_eval.mi`（免语言写的解释器）能解释 `boot_lex.mi` 源码本身，调出 `lexMian` 切 `"let x=1;"` 出 5 token。"语言自己吃自己"的完整形态，不再只是"能处理集合数据"。
